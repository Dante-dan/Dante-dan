...
class TelemetryTests(unittest.TestCase):
    ...
    def test_fetch_timestamp_and_url_version_have_distinct_meanings(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn(f'src="{t.RAW_ASSETS}telemetry/stats-dark-', first)
            ...
            self.assertNotIn('?v=', third)
            versions = list((root / 'assets/telemetry').glob('stats-dark-*.svg'))
            self.assertEqual(len(versions), 2)
            self.assertEqual(sum('new data' in p.read_text() for p in versions), 1)
